  ...
  <div class="tags">
    <div class="current out" v-if="!isNewTag && isShow === '-'">
      <div class="div-wrapper">
        <div v-for="(item, index) in this.outDataSource" :class="{ selected: IsSeletedTag(item) >= 0 }" :key="index" @click="toggle(item)">
    ...
    <div class="current in" v-if="!isNewTag && isShow === '+'">
      <div class="div-wrapper">
        <div v-for="(item, index) in this.inDataSource" :class="{ selected: IsSeletedTag(item) >= 0 }" :key="index" @click="toggle(item)">
          <Icon :name="item.svg"></Icon>
          {{ item.name }}
        </div>
        <div class="new">
    ...
    <div class="tag-list" v-if="isNewTag === '+'">
      <div class="tag-wrapper">
        <div v-for="(item, index) in newDataSource" :key="index" :class="{ selected: fuckTags.indexOf(item) >= 0 }" @click="toggle(item)">
          <Icon :name="item"></Icon>
        </div>
      </div>
    </div>
...
export default class Tags extends Vue {
  ...
  get outDataSource() {
    ...
    return this.$store.state.tagList.filter((item: Tag) => item.type === "-");
  }
