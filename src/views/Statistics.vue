  ...
  export default class Statistics extends Vue {
    ...
    get groupedList() {
      ...
      const copyRecordList = clone(this.recordList)
        .filter((record: Tag) => record.type === this.type)
    ...
    get totalAmount() {
      const copyRecordList = clone(this.recordList);
	  console.log(copyRecordList)
      const allOutRecords = copyRecordList.filter((item: RecordItem) => item.type === '-');
      const allInRecords = copyRecordList.filter((item: RecordItem) => item.type === '+');

	  console.log(allOutRecords)
      const outAmount = allOutRecords.reduce((sum: number, item: RecordItem) => {
        return sum + parseInt(item.amount);
      }, 0);

      const inAmount = allInRecords.reduce((sum: number, item: RecordItem) => {
        return sum + parseInt(item.amount);
      }, 0);
